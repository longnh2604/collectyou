...
class PixelTool: SketchTool {
    ...
    func draw() {
        return autoreleasepool{
            ...
            let ptinImg = CGPoint(x: touchPoint.x * UIScreen.main.scale, y: touchPoint.y * UIScreen.main.scale)
            let img = UIImage(cgImage: cgimg)
            if let imgFilled = img.pixellated(ptinImg) {
                imgFilled.draw(in: CGRect(x: 0,
                                          y: 0,
                                          width: CGFloat(context.width) / UIScreen.main.scale,
                                          height: CGFloat(context.height) / UIScreen.main.scale))
...
class PixelRectTool: SketchTool {
    var lineWidth: CGFloat
    var lineAlpha: CGFloat
    var lineColor: UIColor
    var firstPoint: CGPoint
    ...
    func endFromPoint(_ startPoint: CGPoint, toPoint endPoint: CGPoint) {
        self.lastPoint = endPoint
        self.isEnd = true
    }
    ...
    internal func draw() {
        return autoreleasepool{
            ...
            if self.isEnd {
                guard let cgimg = context.makeImage() else { return  }
                var rectToFill: CGRect!
                if (lastPoint.x - self.firstPoint.x) > 0 {
                    if (lastPoint.y - firstPoint.y) > 0 {
                        rectToFill = CGRect(x: firstPoint.x * UIScreen.main.scale,
                                            y: firstPoint.y * UIScreen.main.scale,
                                            width: (lastPoint.x - self.firstPoint.x) * UIScreen.main.scale,
                                            height: (lastPoint.y - firstPoint.y) * UIScreen.main.scale)
                    } else {
                        rectToFill = CGRect(x: firstPoint.x * UIScreen.main.scale,
                                            y: lastPoint.y * UIScreen.main.scale,
                                            width: (lastPoint.x - self.firstPoint.x) * UIScreen.main.scale,
                                            height: (firstPoint.y - lastPoint.y) * UIScreen.main.scale)
                    }
                } else {
                    if (lastPoint.y - firstPoint.y) > 0 {
                        rectToFill = CGRect(x: lastPoint.x * UIScreen.main.scale,
                                            y: firstPoint.y * UIScreen.main.scale,
                                            width: (firstPoint.x - self.lastPoint.x) * UIScreen.main.scale,
                                            height: (lastPoint.y - firstPoint.y) * UIScreen.main.scale)
                    } else {
                        rectToFill = CGRect(x: lastPoint.x * UIScreen.main.scale,
                                            y: lastPoint.y * UIScreen.main.scale,
                ...
                let img = UIImage(cgImage: cgimg)
                if let imgFilled = img.pixellated(rectToFill) {
                    imgFilled.draw(in: CGRect(x: 0,
                                              y: 0,
                                              width: CGFloat(context.width) / UIScreen.main.scale,
                                              height: CGFloat(context.height) / UIScreen.main.scale))
