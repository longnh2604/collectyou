...
class DrawingVC: UIViewController {
    ...
    @IBOutlet weak var viewCurrColor: RoundUIView!
    ...
    override func viewDidDisappear(_ animated: Bool) {
        drawingPanelView = nil
        imageViewToPan = nil
        lastPanPoint = nil
        imageData = nil
        photoData = nil
        lastTextViewTransform = nil
        lastTextViewTransCenter = nil
        lastTextViewFont = nil
        activeTextView = nil
        textField = nil
        textLocation = nil
        carteIDTemp = nil
    ...
    fileprivate func setupLayout() {
        let button = LGButton()
        button.leftImageSrc = UIImage(named: "icon_drawing_white.png")
        button.titleString = LocalizationSystem.sharedInstance.localizedStringForKey(key: "Drawing", comment: "")
    ...
    fileprivate func setupCanvas() {
        ...
        if onComparison == true {
            ...
            case 768:
                ...
                self.sketchView.loadImage(image: photoData!,topTrans: false,x:0,y:0)
            case 1152:
        ...
        //set curr view color
        self.viewCurrColor.backgroundColor = self.sketchView.lineColor
    ...
    @IBAction func onSave(_ sender: UIButton) {
        ...
        if stickerView.subviews.count > 0 {
            ...
            onSaveImageData(image: (sketchView.image?.resizeImage(targetSize: CGSize(width: self.sketchView.frame.width, height: self.sketchView.frame.height)))!)
        }
    }
    
    @IBAction func onOpenTool(_ sender: UIButton) {
        
        if let currColor = sender.backgroundColor {
            if currColor == UIColor.lightGray {
                btnExpand.backgroundColor = COLOR_SET.kCOMPARE_TOOL_SELECT
                btnExpand.setImage(#imageLiteral(resourceName: "icon_open"), for: .normal)
                
                popTip.hide()
            } else {
                btnExpand.backgroundColor = UIColor.lightGray
                btnExpand.setImage(UIImage(named: "icon_close"), for: .normal)
                
                if let penTool = penToolSelected {
                    drawingPanelView = DrawingPanelView.instanceFromNib(color: sketchView.lineColor,penSize:sketchView.lineWidth,penOpacity: sketchView.lineAlpha,penTool: penTool, delegate: self)
                } else {
                    drawingPanelView = DrawingPanelView.instanceFromNib(color: sketchView.lineColor,penSize:sketchView.lineWidth,penOpacity: sketchView.lineAlpha,penTool: 0, delegate: self)
                }
            
                popTip.show(customView: drawingPanelView, direction: .left, in: view, from: sender.frame)
                popTip.bubbleColor = COLOR_SET.kNAVIGATION_BAR_COLOR
                popTip.shouldDismissOnTap = false
                popTip.tapOutsideHandler = { _ in
                    self.btnExpand.backgroundColor = COLOR_SET.kCOMPARE_TOOL_SELECT
                    self.btnExpand.setImage(#imageLiteral(resourceName: "icon_open"), for: .normal)
                    self.popTip.hide()
                }
            }
        }
...
extension DrawingVC: DrawingPanelViewDelegate {
    ...
    func onColorChange(color: UIColor) {
        if let rgb = color.rgbValue {
            self.sketchView.lineColor = UIColor(red: rgb.r, green: rgb.g, blue: rgb.b, alpha: 1)
            viewCurrColor.backgroundColor = self.sketchView.lineColor
        }
    }
    ...
    func onIllegalInput() {
        Utils.showAlert(message: "正しく記入してください", view: self)
    }
