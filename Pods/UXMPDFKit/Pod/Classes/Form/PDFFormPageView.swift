...
open class PDFFormPage: NSObject {
    ...
    func renderInContext(_ context: CGContext, size: CGRect) {
        let formView = PDFFormPageView(
            frame: CGRect(x: size.origin.x, y: size.origin.y, width: size.width, height: size.height),
            boundingBox: CGRect(x: size.origin.x, y: size.origin.y, width: size.width, height: size.height),
            cropBox: CGRect(x: size.origin.x, y: size.origin.y, width: size.width, height: size.height),
            fields: self.fields)
